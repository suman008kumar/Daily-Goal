...
export const detectFace = async (videoElement) => {
  ...
  if (videoElement.currentTime !== lastVideoTime) {
    ...
    lastResult = model.detectForVideo(videoElement, safeTimestamp);
    lastVideoTime = videoElement.currentTime;
  }
  const faces = lastResult?.faceLandmarks || [];
  return { detected:faces.length>0, confidence:confidenceFromLandmarks(faces[0]), status:faces.length>0?"detected":"not_detected", landmarks:faces[0]||[], faceCount:faces.length, blendshapes:lastResult?.faceBlendshapes?.[0]?.categories||[], timestamp: lastTimestamp || timestamp };
};

export const resetFaceModel = () => { landmarker?.close?.(); landmarker=null; loadingPromise=null; lastVideoTime=-1; lastResult=null; lastTimestamp=0; status=FACE_MODEL_STATUS.UNAVAILABLE; };
